## Cycle — 2026-02-17 09:51 America/Toronto
_Auditor_: IdleWatch iOS QA Cron
_Scope_: QA cycle for `idlewatch-ios` Flutter app, with emphasis on UX/Auth/Onboarding/Performance status refresh.
_Method_: Flutter analyze + Flutter test + prior backlog reconciliation.

### Summary
- ✅ `flutter analyze`: **No issues found**.
- ✅ `flutter test`: **All tests passed** (all 19 tests).
- ✅ No runtime code changes in this cycle; validation confirms existing onboarding/auth/perf UX invariants are intact.
- ⚠️ No new older/slower-device performance capture executed in this cycle (pending hardware/profile availability).

### Prioritized Issues (UX/Auth/Onboarding/Performance)

#### P1 — Maintain remote iOS smoke closure evidence
- **Status**: ✅ Closed
- **Current state**: Latest green smoke reference remains current: `https://github.com/bestquark/idlewatch-ios/actions/runs/22092064460`.
- **Acceptance criteria**:
  - Keep the latest green iOS Smoke run URL updated in this log when flow, test, or workflow changes occur.
  - If onboarding/auth behavior changes materially, add a fresh smoke artifact reference in the same cycle.

#### P2 — Preserve onboarding/auth clarity and recovery UX
- **Status**: ✅ Closed
- **Current state**: Sign-in helper/retry timing + onboarding recovery states remain implemented and validated by tests.
- **Acceptance criteria**:
  - Onboarding/auth sequences continue to present explicit helper and retry states for signing-in and recovery.
...
- **Status**: ✅ Closed
- **Current state**: Timing policy, runtime signal names, and deterministic tests for helper/retry behavior remain in place and passing.
- **Acceptance criteria**:
  ...
  - Confirm each cycle that 10s helper and 30s retry behavior are still tested and pass.

#### P4 — Expand older/low-end onboarding/performance coverage
- **Status**: ⚠️ Partial
- **Current state**: No older/slower-device perf capture executed in this cycle. The lower-end simulator candidate is already identified (`iPhone 16e`) but the target capture is still pending.
- **Acceptance criteria**:
...
_Method_: Flutter analyze + Flutter test + existing issue backlog reconciliation.

### Summary
- ✅ `flutter analyze`: **No issues found**.
- ✅ `flutter test`: **All tests passed** (all 19 tests).
- ℹ️ Cycle was docs-only; no application/runtime code changes were required this pass.
- ✅ Priorities and acceptance criteria were refreshed; no new blocker found.
- ℹ️ Explicit older/slower-device performance capture remains pending due available hardware constraints.
...
#### P4 — Expand older/low-end onboarding/performance coverage
- **Status**: ⚠️ Partial
- **Current state**: No new older/slower iOS capture executed in this cycle; earlier simulator captures still represent the newest perf evidence.
- **Acceptance criteria**:
  - Execute at least one older/slower iOS profile capture.
  - Record and evaluate values for:
    - `bootstrap_init_ms`
...
_Method_: Flutter analyze + Flutter test + QA backlog reconciliation.

### Summary
- ✅ `flutter analyze`: **No issues found**.
- ✅ `flutter test`: **All tests passed** (all 17 tests).
- ✅ This cycle remained docs-maintenance; no app behavior/code changes required.
- ✅ Existing smoke evidence link remains stable (`https://github.com/bestquark/idlewatch-ios/actions/runs/22092064460`).
- ⚠️ P4 remains open for explicit older/low-end iOS profiling coverage.

### Prioritized Issues (UX/Auth/Onboarding/Performance)

#### P1 — Maintain remote iOS smoke closure evidence
- **Status**: ✅ Closed
- **Current state**: Latest known green smoke URL remains unchanged and current in log.
- **Acceptance criteria**:
  - Keep the latest green iOS Smoke run URL updated in this log on meaningful app or workflow changes.
  - If onboarding/auth behavior changes, capture and add a fresh smoke artifact in the same cycle.
...
#### P2 — Preserve onboarding/auth clarity and recovery UX
- **Status**: ✅ Closed
- **Current state**: Auth helper/retry/visibility behavior remains intact and verified by tests; no UX copy changes observed.
- **Acceptance criteria**:
  - Onboarding/auth sequences continue to expose clear states for helper visibility and retry behavior.
  - Any changes to auth/onboarding copy/flow must update this log with evidence links.
...
#### P3 — Maintain onboarding/performance guardrails and thresholds
- **Status**: ✅ Closed
- **Current state**: Timing policy + deterministic tests for 10s helper / 30s retry behavior and dashboard signals remain in place and green.
- **Acceptance criteria**:
  - Keep guardrail signal names and expected thresholds discoverable in docs/artifacts/tests.
  - Update this log whenever thresholds or test evidence changes.

#### P4 — Expand older/low-end onboarding/performance coverage
- **Status**: ⚠️ Partial
- **Current state**: No new lower-end iOS/perf capture captured this cycle.
- **Acceptance criteria**:
  - Run at least one older/slower iOS profile capture.
  - Record and evaluate: `bootstrap_init_ms`, `auth_sign_in_ms`, `dashboard_first_render_ms`, `loading_helper`, `loading_retry`, `dashboard_retry_recovery_ms` against guardrails or documented exception.
...
_Method_: Static analysis + local test-run probe + existing issue-log reconciliation.

### Summary
- ✅ `flutter analyze` passed cleanly.
- ⚠️ `flutter test` started and hung at `test/activity_normalization_test.dart` startup; terminated after a timeout to avoid blocking.
- ℹ️ No runtime/app behavior changes in this cycle; docs-only update.
- ✅ Highest-priority backlog remains unchanged from previous cycle: `P1` + `P2` + `P3` closed, `P4` remains partial.

### Prioritized Issues (UX/Auth/Onboarding/Performance) with Acceptance Criteria

#### P1 — Keep remote iOS smoke closure evidence and regression signal current
- **Status**: ✅ Closed
- **Current state**: Remote iOS Smoke run reference remains valid and is still the gating artifact for this area.
- **Acceptance criteria**:
  - QA log continues to track the most recent green iOS Smoke run URL.
  - Any observed behavior change in onboarding/auth/perf should be accompanied by a fresh smoke artifact reference in the same cycle.

#### P2 — Maintain onboarding/auth flow clarity and recovery behavior
- **Status**: ✅ Closed
- **Current state**: Delayed-helper/retry UX contract remains implemented in deterministic tests and runtime policy docs.
- **Acceptance criteria**:
  - Auth onboarding path still demonstrates clear progress/failure-recovery states (`signing in`, helper, retry) in logs or tests.
  - Regression risk remains low; any onboarding copy/flow changes must update evidence links and acceptance checks in this log.

#### P3 — Preserve onboarding/performance guardrails and measurable thresholds
- **Status**: ✅ Closed
- **Current state**: Guardrails for bootstrap/auth/dashboard timing remain in policy/tests and are still expected to fire consistently.
- **Acceptance criteria**:
  - Keep signal names and thresholds discoverable in QA artifacts (`idlewatch-perf`-family/`runtime` evidence).
  - If timing thresholds are changed, update this log with new expected maxes and test references in the same cycle.

#### P4 — Expand older/low-end device onboarding/perf confidence
- **Status**: ⚠️ Partial
- **Current state**: No new lower-end capture in this cycle.
- **Acceptance criteria**:
  - Execute at least one older/slower iOS profile capture.
  - Record in artifact + log values for:
    - `bootstrap_init_ms`
    - `auth_sign_in_ms`
    - `dashboard_first_render_ms`
    - `loading_helper`
    - `loading_retry`
    - `dashboard_retry_recovery_ms`
  - Move `P4` to ✅ Closed once all values are in-budget or a formal exception is approved.

### Validation Notes
- `flutter analyze`: ✅
- `flutter test`: ⚠️ (start hang at `activity_normalization_test.dart`)
- Commit scope: docs-only (`docs/qa/ios-qa-log.md`).
